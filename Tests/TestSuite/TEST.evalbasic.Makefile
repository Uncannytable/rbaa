...
Output/%.$(TEST).report.txt: Output/%.linked.rbc $(LOPT) \
	...
	@~/Ecosoc/llvm-3.5.0/lib/Ecosoc/llvm-sra/SAGE/bin/sage-opt -load Python.so -load SAGE.so -load SRA.so -load RangeBasedAliasAnalysis.so -redef -basicaa -aa-eval $<.ssify.bc -o $<.redef.bc 2>>$@ 
