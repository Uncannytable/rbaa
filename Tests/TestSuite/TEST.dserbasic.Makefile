...
Output/%.$(TEST).report.txt: Output/%.linked.rbc $(LOPT) \
	...
	@~/Ecosoc/llvm-3.5.0/lib/Ecosoc/llvm-sra/SAGE/bin/sage-opt -load Python.so -load SAGE.so -load SRA.so -load RangeBasedAliasAnalysis.so -basicaa -range-based-aa -dse -stats $<.ssify.bc -o $<.redef.bc 2>>$@ 
