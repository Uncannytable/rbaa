...
#include <iostream>
#include <string>
#include "../rbaaV2/RangeBasedAliasAnalysis.h"
using namespace llvm;

#define DEBUG_TYPE "licm" //ici
...
static cl::opt<bool>
DisablePromotion("disable-licm2-promotion", cl::Hidden,//ici
                 cl::desc("Disable memory promotion in LICM pass"));


namespace {
  struct LICM : public LoopPass { //Maroua LICM => LICM 
    static char ID; // Pass identification, replacement for typeid
    LICM() : LoopPass(ID) { //Maroua LICM() => LICM() 
      //initializeLICMPass(*PassRegistry::getPassRegistry()); //Maroua  Initialize all passes linked into the ScalarOpts library
    }
    ...
    void getAnalysisUsage(AnalysisUsage &AU) const override {
	  ...
	  //AU.addRequired<RangeBasedAliasAnalysis>();
    }
	...
	errs() <<"===================================load insts hoisted or sunk=====================================\n" ;
	for (std::vector<std::string>::const_iterator i = tab.begin(); i != tab.end(); ++i) errs() << *i << "\n";
	errs() <<"==================================================================================================\n \n \n";

	errs() <<"===================================call insts hoisted or sunk=====================================\n" ;
	for (std::vector<std::string>::const_iterator i = tabCall.begin(); i != tabCall.end(); ++i) errs() << *i << "\n";
	errs() <<"==================================================================================================\n \n \n";

	 errs() <<"===================================Instruction sunk===============================================\n" ;	
	for (std::vector<std::string>::const_iterator i = tabSink.begin(); i != tabSink.end(); ++i) errs() << *i << "\n";
	errs() <<"==================================================================================================\n \n \n";

	errs() <<"===================================Instruction hoisted============================================\n" ;	
	for (std::vector<std::string>::const_iterator i = tabHoisted.begin(); i != tabHoisted.end(); ++i) errs() << *i << "\n";
	errs() <<"==================================================================================================\n \n \n";

	errs() <<"===================================Instruction promoted==========================================\n" ;
	for (std::vector<std::string>::const_iterator i = tabPromoted.begin(); i != tabPromoted.end(); ++i) errs() << *i << "\n";
	errs() <<"==================================================================================================\n \n \n";
  ...
  private:
	...
	std::string hoisted = "";  //Maroua	
	std::string ccc = "";  //Maroua
	std::vector<std::string> tab; //Maroua
	std::vector<std::string> tabSink; //Maroua
	std::vector<std::string> tabPromoted; //Maroua
	std::vector<std::string> tabHoisted; //Maroua
	std::vector<std::string> tabCall; //Maroua
	//std::ostream st;
	//llvm::raw_string_ostream I4 (hoisted);
    ...
    bool pointerInvalidatedByLoop(Value *V, uint64_t Size,
      ...
      // Check to see if any of the basic blocks in CurLoop invalidate *V.
		//sets the value to true if a new alias set is created to contain the pointer (no alias)
errs()<< "\n getAliasSetForPointerIfExists : " << *(CurAST->getAliasSetForPointerIfExists(V, Size, TBAAInfo)) << "\n \n"; //Maroua 
...
char LICM::ID = 0;
INITIALIZE_PASS_BEGIN(LICM, "licm2", "Loop Invariant Code Motion", false, false)
...
INITIALIZE_PASS_END(LICM, "licm2", "Loop Invariant Code Motion", false, false)
...
errs() << "CurLoop Preheader :		" << *Preheader << "\n \n";//Maroua
  ...
  if (!DisablePromotion && (Preheader || L->hasDedicatedExits())) {
    ...
    for (AliasSetTracker::iterator I = CurAST->begin(), E = CurAST->end();
         I != E; ++I)
		{
		errs() <<"curAST tracker object, AS promoted" << *I << "\n" ;	//Maroua
...
errs() << *C <<"est une instruction dont les operandes sont des constantes\n ";//Maroua
      ...
      //
		errs() << "\n CurLoop->hasLoopInvariantOperands(&I) for" << I << ", gives : " << CurLoop->hasLoopInvariantOperands(&I) << "\n" ;
		errs() << " canSinkOrHoistInst for" << I << " gives : " << canSinkOrHoistInst(I) << "\n" ;
		errs() << " isSafeToExecuteUnconditionally for" << I << ", gives : " << isSafeToExecuteUnconditionally(I) << "\n" ;
		//errs() << "operand 0 " << *((&I)->getOperand(0)) << "\n";
		//errs() << "operand 1 " << *((&I)->getOperand(1)) << "\n";
      if (CurLoop->hasLoopInvariantOperands(&I) && canSinkOrHoistInst(I) &&
          isSafeToExecuteUnconditionally(I)){
		
		//I.print(errs());        
		hoist(I);}
	else errs() << "don't hoist (" << I << ")\n";	//Maroua
    }
...
bool LICM::canSinkOrHoistInst(Instruction &I) {
  ...
  if (LoadInst *LI = dyn_cast<LoadInst>(&I)) {
    if (!LI->isUnordered()){
		errs() << "don't hoist (" << I << ") because volatile/atomic loads\n";	//Maroua
      return false;        // Don't hoist volatile/atomic loads!
					}
    // Loads from constant memory are always safe to move, even if they end up
    // in the same alias set as something that ends up being modified.

    if (AA->pointsToConstantMemory(LI->getOperand(0))){ errs()<<"grrrrrr\n";
      return true;}
    if (LI->getMetadata("invariant.load")) { errs()<<"grrrrrr22222\n";
      return true;}
...
//errs() << "Instruction to sink" << I << "\n";	//Maroua
  DEBUG(dbgs() << "LICM sinking instruction: " << I << "\n");
	std::string hoisted = "";  //Maroua	
	llvm::raw_string_ostream I3 (hoisted);
		I.print(I3);
		tabSink.push_back(I3.str()); //Maroua

  if (isa<LoadInst>(I)) {
		++NumMovedLoads;
		std::string hoisted = "";  //Maroua	
		llvm::raw_string_ostream I2 (hoisted);
		I.print(I2);
		tab.push_back(I2.str()); //Maroua;
	}
  else if (isa<CallInst>(I)) {
		++NumMovedCalls;
		std::string hoisted = "";  //Maroua	
		llvm::raw_string_ostream I6 (hoisted);
		I.print(I6);
		tabCall.push_back(I6.str()); //Maroua;
	}
  ++NumSunk;
  Changed = true;
...
void LICM::hoist(Instruction &I) {
  ...
  if (isa<LoadInst>(I)) {
		++NumMovedLoads;
		std::string hoistedload = "";  //Maroua	
		llvm::raw_string_ostream I2 (hoistedload);
		I.print(I2);
		tab.push_back(I2.str()); //Maroua
		errs()<<"load hoisted"<<I<<"\n";
	}
		...
		++NumMovedCalls;
		std::string hoistedcall = "";  //Maroua	
		llvm::raw_string_ostream I6 (hoistedcall);
		I.print(I6);
		tabCall.push_back(I6.str()); //Maroua;
	}
  ++NumHoisted;
	std::string hoisted = "";  //Maroua	
	llvm::raw_string_ostream I5 (hoisted);
	I.print(I5);
	tabHoisted.push_back(I5.str()); //Maroua
...
void LICM::PromoteAliasSet(AliasSet &AS,
  ...
  if (AS.isForwardingAliasSet() || !AS.isMod() || !AS.isMustAlias() ||
	...
	errs()<< "I can not promote this AS" << AS << "\n"; 
    return;}
  ...
  ++NumPromoted;	
	...
	llvm::raw_string_ostream I4 (hoisted);
		(*SomePtr).print(I4);
		tabPromoted.push_back(I4.str()); //Maroua
...
//Maroua adds...
//char LICM::ID = 0;
static RegisterPass<LICM> X("licm2", "LICM");
